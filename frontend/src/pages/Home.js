...
/**
 * Formats Date objects as YYYY-MM-DD using local time.
 * This avoids shifting spoken dates by one day when converting through UTC.
 */
...
function Home() {
  ...
  const handleActivateAssistant = () => {
    ...
    const welcome =
      "Welcome to BookMyTable! I'm your digital assistant. How can I help you today?";
  ...
  const handleUserMessage = async (message) => {
    ...
    if (step === -1) {
      if (
        ...
        const r =
          "I am a booking specialist for BookMyTable. Would you like to reserve a table?";
        setAgentMessage(r);
        speak(r);
      }
    }
    ...
    // STEP 7: Final Confirmation and Database Sync
      ...
      if (input.includes("yes") || input.includes("confirm")) {
        ...
        try {
          ...
          speak(
            "Great news! Your reservation at BookMyTable is confirmed. We look forward to seeing you!",
          );
          setIsSuccess(true);
          setAssistantActivated(false);
        } catch (e) {
  ...
  return (
    <div className="hero-wrapper">
      ...
      <footer className="home-footer">
        <p>
          BookMyTable | Fast & Simple Table Reservations | BUILT BY ADITYA VIKRAM
        </p>
      </footer>
    </div>
  );
